...
pub fn new_external_bus() -> (
    broadcast::Sender<protocol::Message>,
    broadcast::Receiver<protocol::Message>,
) {
...
///   original [`protocol::Message::BusSecure`] that could not be decrypted.
///   The caller should buffer this message and replay it onto the external bus
///   once the session is unlocked, so the message is not permanently lost.
pub fn spawn_decrypt_middleware(
    on_bus_secure: impl Fn(&protocol::BusSecureEnvelope) -> DecryptResult + Send + 'static,
    on_locked: impl Fn(protocol::Message) + Send + 'static,
    mut external_rx: broadcast::Receiver<protocol::Message>,
    internal_tx: broadcast::Sender<InboundBusMessage>,
) -> tokio::task::JoinHandle<()> {
    tokio::spawn(async move {
        loop {
            let raw_msg = match external_rx.recv().await {
                Ok(msg) => msg,
