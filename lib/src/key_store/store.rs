...
impl KeyStore {
    pub fn open(dir: &Path) -> Result<Self> {
        fs::create_dir_all(dir)
            .with_context(|| format!("Cannot create key store at {:?}", dir))?;
    ...
    pub fn pub_file_path(&self, fingerprint: &str) -> PathBuf { self.pub_path(fingerprint) }
    pub fn sec_file_path(&self, fingerprint: &str) -> PathBuf { self.sec_path(fingerprint) }

    fn pub_path(&self, fp: &str) -> PathBuf { self.dir.join(format!("{}.pub.asc", fp)) }
    fn sec_path(&self, fp: &str) -> PathBuf { self.dir.join(format!("{}.sec.asc", fp)) }
    fn index_path(&self) -> PathBuf { self.dir.join("index.json") }

    fn find_entry(&self, query: &str) -> Result<KeyEntry> {
        let q = query.to_lowercase();
        let matches: Vec<_> = self.list()?
            .into_iter()
            .filter(|e| e.fingerprint.to_lowercase().contains(&q) || e.uid.to_lowercase().contains(&q))
            .collect();
        match matches.len() {
    ...
    fn update_index(&self, cert: &Cert) -> Result<()> {
        ...
        entries.retain(|e| e.fingerprint != fp);
        entries.push(KeyEntry { fingerprint: fp, uid, algo, has_secret: cert.is_tsk() });
        fs::write(self.index_path(), serde_json::to_string_pretty(&entries)?)?;
        Ok(())
    }
}
...
fn decrypt_all_secrets(cert: Cert, passphrase: &str) -> Result<Cert> {
    ...
    for ka in cert.keys().subkeys().secret() {
        let key = ka.key().clone()
            .parts_into_secret().context("Subkey has no secret material")?;
