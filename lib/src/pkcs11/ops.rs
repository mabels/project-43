...
pub(crate) fn load_cert(path: &Path) -> Result<Cert> {
    Cert::from_file(path).with_context(|| format!("Failed to load public key from {:?}", path))
}

fn touch_prompt() {
    eprintln!("Touch YubiKey now...");
}
...
pub fn decrypt_with_card(data: &[u8], pin: &str, ident: Option<&str>) -> Result<Vec<u8>> {
    ...
    let decryptor = user_card
        .decryptor(&touch_prompt)
        .context("Failed to get decryptor from card")?;
...
pub fn sign_encrypt(data: &[u8], recipient_path: &Path, pin: &str) -> Result<String> {
    ...
    let mut output = Vec::new();
    {
        ...
        let message = Encryptor2::for_recipients(message, recipient_keys)
            .build()
            .context("Failed to build encryptor")?;
        let signed = Signer::new(message, signer)
            .build()
