...
fn decrypt_all_secrets(cert: Cert, passphrase: &str) -> Result<Cert> {
    ...
    let primary = cert.primary_key().key().clone()
        .parts_into_secret().context("Primary key has no secret material")?;
    packets.push(primary.decrypt_secret(&pw).context("Failed to decrypt primary key")?.into());
    for ka in cert.keys().subkeys().secret() {
        let key = ka.key().clone().parts_into_secret().context("Subkey has no secret material")?;
        packets.push(key.decrypt_secret(&pw).context("Failed to decrypt subkey")?.into());
    }
    cert.insert_packets(packets).context("Failed to rebuild cert with decrypted secrets")
}

pub fn load_secret_cert(key_file: &Path, passphrase: &str) -> Result<Cert> {
    let cert = load_cert(key_file)?;
    if passphrase.is_empty() { Ok(cert) } else { decrypt_all_secrets(cert, passphrase) }
}

// ── SoftDecryptHelper ─────────────────────────────────────────────────────────

struct SoftDecryptHelper { cert: Cert }

impl VerificationHelper for SoftDecryptHelper {
    fn get_certs(&mut self, _ids: &[KeyHandle]) -> openpgp::Result<Vec<Cert>> { Ok(vec![]) }
    fn check(&mut self, _structure: MessageStructure) -> openpgp::Result<()> { Ok(()) }
}

impl DecryptionHelper for SoftDecryptHelper {
    fn decrypt<F>(&mut self, pkesks: &[PKESK], _skesks: &[SKESK], sym_algo: Option<SymmetricAlgorithm>, mut decrypt_fn: F) -> openpgp::Result<Option<Fingerprint>>
        ...
        for pkesk in pkesks {
            for ka in self.cert.keys().with_policy(policy, None).for_transport_encryption().secret() {
                ...
                if let Some((algo, sk)) = pkesk.decrypt(&mut keypair, sym_algo) {
                    if decrypt_fn(algo, &sk) { return Ok(Some(ka.fingerprint())); }
                }
            }
        }
...
struct VerifyDecryptHelper { decrypt_cert: Cert, verify_cert: Cert }

impl VerificationHelper for VerifyDecryptHelper {
    fn get_certs(&mut self, _ids: &[KeyHandle]) -> openpgp::Result<Vec<Cert>> { Ok(vec![self.verify_cert.clone()]) }
...
impl DecryptionHelper for VerifyDecryptHelper {
    ...
    where F: FnMut(SymmetricAlgorithm, &SessionKey) -> bool {
        let policy = &StandardPolicy::new();
        for pkesk in pkesks {
            for ka in self.decrypt_cert.keys().with_policy(policy, None).for_transport_encryption().secret() {
                let mut keypair = ka.key().clone().into_keypair()?;
                if let Some((algo, sk)) = pkesk.decrypt(&mut keypair, sym_algo) {
                    if decrypt_fn(algo, &sk) { return Ok(Some(ka.fingerprint())); }
                }
...
pub fn sign(data: &[u8], key_file: &Path, passphrase: &str) -> Result<String> {
    ...
    let keypair = cert.keys().with_policy(policy, None).for_signing().secret().next()
    ...
    let mut output = Vec::new();
    {
        ...
        let mut signer = Signer::new(message, keypair).detached().build().context("Failed to build signer")?;
...
pub fn decrypt(data: &[u8], key_file: &Path, passphrase: &str) -> Result<Vec<u8>> {
    let policy = &StandardPolicy::new();
    let helper = SoftDecryptHelper { cert: load_secret_cert(key_file, passphrase)? };
    let mut output = Vec::new();
    let mut decryptor = DecryptorBuilder::from_reader(io::BufReader::new(data))?
        .with_policy(policy, None, helper).context("Decryption failed")?;
    io::copy(&mut decryptor, &mut output)?;
    Ok(output)
}

pub fn sign_encrypt(data: &[u8], key_file: &Path, recipient_path: &Path, passphrase: &str) -> Result<String> {
    let policy = &StandardPolicy::new();
    let cert = load_secret_cert(key_file, passphrase)?;
    ...
    let mut output = Vec::new();
    {
        ...
        let signed = Signer::new(message, keypair).build().context("Failed to build signer")?;
...
pub fn decrypt_verify(data: &[u8], key_file: &Path, signer_path: &Path, passphrase: &str) -> Result<Vec<u8>> {
    let policy = &StandardPolicy::new();
