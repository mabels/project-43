...
// `busnum()` is Linux-only in nusb; derive an equivalent u8 on other platforms.
#[cfg(target_os = "linux")]
#[inline]
fn device_busnum(info: &nusb::DeviceInfo) -> u8 {
    info.busnum()
}
#[cfg(not(target_os = "linux"))]
#[inline]
fn device_busnum(info: &nusb::DeviceInfo) -> u8 {
    // On macOS / Windows the high byte of location_id encodes the bus.
    (info.location_id() >> 24) as u8
}
...
impl Backend<NUSBDevice, Interface> for NUSBBackend {
    ...
    fn open_device(
        ...
        for device_info in nusb_devices {
            if device_busnum(&device_info) != bus_id || device_info.device_address() != address_id {
                continue;
            }
    ...
    fn find_device_from_serial(&mut self, serial: u32) -> Result<Device, ChallengeResponseError> {
        ...
        for device_info in nusb_devices {
            ...
            let device_serial =
                match self.read_serial_from_device(device_busnum(&device_info), device_info.device_address()) {
                    Ok(s) => s,
                    Err(_) => continue,
                };
            ...
            if device_serial == serial {
                return Ok(Device {
                    ...
                    product_id,
                    vendor_id,
                    bus_id: device_busnum(&device_info),
    ...
    fn find_all_devices(&mut self) -> Result<Vec<Device>, ChallengeResponseError> {
        ...
        for device_info in nusb_devices {
            ...
            let device_serial = self
                .read_serial_from_device(device_busnum(&device_info), device_info.device_address())
                .ok();
